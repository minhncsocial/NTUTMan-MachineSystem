...
BOOL  CImage::Load(const char * csFileName)
{
	BOOL bFlag = FALSE;
	...
	std::cout << "Load(const CString csFileName)" << " | " << csFileName << std::endl;
	//std::wcout << csFileName1 << std::endl;
 	...
 	//else if (! stricmp(ext, ".jpg")) 	
		...
		bFlag=LoadGIF(csFileName);
 	else if (! stricmp(ext, ".pcx")) 	
		bFlag=LoadPCX(csFileName);
 	else if (! stricmp(ext, ".tga")) 	
		bFlag=LoadTGA(csFileName);
   	return(bFlag);
}
