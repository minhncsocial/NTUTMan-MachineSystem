...
BOOL  CImage::Load(const CString csFileName)
{
	...
	_splitpath_s((const char*)(LPCTSTR)csFileName, drive, dir, fname, ext);   

 	if (! _stricmp(ext, ".bmp")) 	
		bFlag=LoadBMP((const char*)(LPCTSTR)csFileName);
 	else if (! _stricmp(ext, ".jpg")) 	
		bFlag=LoadJPEG((const char*)(LPCTSTR)csFileName);
 	else if (! _stricmp(ext, ".tif")) 	
		bFlag=LoadTIF((const char*)(LPCTSTR)csFileName);
 	else if (! _stricmp(ext, ".gif")) 	
		bFlag=LoadGIF((const char*)(LPCTSTR)csFileName);
 	else if (! _stricmp(ext, ".pcx")) 	
		bFlag=LoadPCX((const char*)(LPCTSTR)csFileName);
 	else if (! _stricmp(ext, ".tga")) 	
		bFlag=LoadTGA((const char*)(LPCTSTR)csFileName);
   	return(bFlag);
}
...
BOOL  CImage::Save(const CString csFileName)
{
	...
	_splitpath_s((const char*)(LPCTSTR)csFileName, drive, dir, fname, ext);   
 	...
 	if (! _stricmp(ext, ".bmp")) 	
		...
		bFlag=SaveJPEG((const char*)(LPCTSTR)csFileName);
 	else if (! _stricmp(ext, ".tif")) 	
