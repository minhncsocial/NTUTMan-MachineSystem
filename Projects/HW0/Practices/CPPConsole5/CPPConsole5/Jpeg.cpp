...
// load gif file
BOOL  CImage::LoadJPEG(LPCSTR lpstrFileName)
{
	UINT	uWidth, uHeight;
	BYTE	*lpTmpBuffer;
   	BYTE    *buf1,*buf2;
  	int		i,j,k,nJPEGBytesPerLine;

	lpTmpBuffer = ReadJPEGFile(lpstrFileName, &uWidth, &uHeight);
	if (lpTmpBuffer == NULL)
		return FALSE;
 
	Create(uWidth,uHeight,24,0);
 	
   	buf1=(BYTE*) lpBits+(nHeight-1)*nBytesPerLine;
	buf2=(BYTE*) lpTmpBuffer;
	nJPEGBytesPerLine=3*nWidth;
   	for (j=0;j<nHeight;j++) {
		for (i=0,k=0;i<nWidth;i++,k+=nBytesPerPixel) {
			buf1[k+2]=buf2[k];
			buf1[k+1]=buf2[k+1];
 			buf1[k]  =buf2[k+2];
		}
 		buf1-=nBytesPerLine;
		buf2+=nJPEGBytesPerLine;
	}
...
// save Jpeg file
BOOL  CImage::SaveJPEG(LPCSTR lpstrFileName)
{
	BOOL	bSuccess,bColor;
	BYTE	*tmp,*buf1,*buf2;
	int		i,j,k,Bpl,nJPEGBytesPerLine,nQuality;
      
	if (nBitCount<8) 
		return(FALSE);
  
	Bpl=nBytesPerLine;
	if (nBitCount==8) Bpl *= 3;
	tmp =(BYTE*) malloc(Bpl*nHeight);
   	buf1=(BYTE*) lpBits+(nHeight-1)*nBytesPerLine;
	buf2=(BYTE*) tmp;
	nJPEGBytesPerLine=3*nWidth;
	if (nBitCount==24) {
 	...
 	nQuality=90;
	bSuccess = WriteJPEGFile(lpstrFileName,
							tmp,
							nWidth, 
							nHeight,
							bColor,
...
//	write a JPEG file
//
BOOL  WriteJPEGFile(LPCSTR lpstrFileName, 
						BYTE *dataBuf,
						UINT widthPix,
						UINT height,
						BOOL color, 
					 ...
					 int row)
{
 	...
 	buf=outBuf + 3*widthPix*row;
   	memcpy(buf,jpegline,3*widthPix);
}
...
void j_putGrayScanlineToRGB(BYTE *jpegline, 
							 ...
							 int row)
{
	int		offset = 3 * widthPix * row;
 	int		count,i;
	BYTE	*buf1,*buf2;
	BYTE	iGray;

	buf1=jpegline+widthPix-1;
	buf2=outBuf+offset+3*(widthPix-1);
	for (count=0;count<widthPix;count++) 
	{
 		iGray=(*buf1--);
		for (i=0;i<3;i++) buf2[i]=iGray;
		buf2-=3;
	}
}
